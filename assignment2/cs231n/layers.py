...
def batchnorm_forward(x, gamma, beta, bn_param):
    ...
    if mode == 'train':
        ...
        var = np.var(x, axis = 0)
        x_std = (x - mean)/np.sqrt(var + eps)
        out = gamma * x_std + beta
        running_mean = momentum * running_mean + (1 - momentum) * mean
        running_var = momentum * running_var + (1 - momentum) * var 
        cache = (mean, var, x, x_std, gamma, beta,eps)
        #######################################################################
        #                           END OF YOUR CODE                          #
        #######################################################################
    elif mode == 'test':
...
def batchnorm_backward(dout, cache):
    ...
    # might prove to be helpful.                                              #
    ###########################################################################
    mean, var, x, x_std, gamma, beta, eps = cache
    m = x.shape[0]
    dx_std = dout *  gamma
    dvar = np.sum(dx_std * (x - mean),axis = 0) * (-1/2) * ((var + eps)**(-3/2))
    dmean = np.sum(dx_std * (-1/np.sqrt(var + eps)),axis = 0) + dvar*np.sum(-2 *(x - mean),axis=0)/m
    dx = dx_std/np.sqrt(var + eps) + 2 * dvar * (x - mean)/m + dmean/m 
...
def batchnorm_backward_alt(dout, cache):
    ...
    dx = (1. / m) * gamma * (var + eps)**(-1. / 2.) * (m * dout - np.sum(dout, axis=0) - (x - mean) * (var + eps)**(-1.0) * np.sum(dout * (x - mean), axis=0))
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def layernorm_backward(dout, cache):
    ...
    x_std = x_std.T
    m = x_std.shape[0]
    dx = 1.0/m * 1/np.sqrt(var + eps) * (m*dx_std - np.sum(dx_std, axis=0) - x_std*np.sum(dx_std*x_std, axis=0))
    dx =dx.T
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    # compute the output demension
    N,C,H,W = x.shape
    F,C,HH,WW = w.shape
    n_H = int((H - HH + 2 * pad) / stride) + 1
    n_W = int((H - WW + 2 * pad) / stride) + 1
...
def conv_backward_naive(dout, cache):
    ...
    # define the gradient of all params
    dx = np.zeros_like(x)
    dw = np.zeros_like(w)
    db = np.zeros_like(b)
    ...
    # following the instructions by Andrew Ng in Coursera course
    for i in range(N):
        x_prev_pad = x_pad[i]
        dx_prev_pad = dx_pad[i]
        for h in range(n_H):
            for d in range(n_W):
                for c in range(F):
                    # define the corners of current slice
                    vert_start = h*stride
                    vert_end = vert_start + HH
                    horiz_start = d * stride
                    horiz_end = horiz_start + WW
                    ...
                    dx_prev_pad[:,vert_start:vert_end,horiz_start:horiz_end] += w[c,:,:,:] * dout[i,c,h,d]
                    dw[c,:,:,:] += x_slice * dout[i,c,h,d]
                    db[c] +=dout[i,c,h,d]  
        dx[i] = dx_prev_pad[:,pad:-pad,pad:-pad]
...
def max_pool_forward_naive(x, pool_param):
    ...
    n_H = 1 + int((H - HH)/stride)
    n_W = 1 + int((W - WW)/stride)
        ...
        for h in range(n_H):
            for d in range(n_W):
                for c in range(C):
                    ...
                    horiz_start = d * stride
                    horiz_end = horiz_start +WW
                    x_prev_slice = x[i,c,vert_start:vert_end,horiz_start:horiz_end]
                    out[i,c,h,d] = np.max(x_prev_slice)
        ...
        for h in range(H):
            for d in range(W):
                for c in range(C):
                    ...
                    horiz_start = d * stride
                    horiz_end = horiz_start + WW 
                    x_prev_slice = x_prev[c,vert_start:vert_end,horiz_start:horiz_end]
                    mask = (x_prev_slice == np.max(x_prev_slice))
                    dx[i,c,vert_start:vert_end,horiz_start:horiz_end] += mask*dout[i,c,h,d] 
    ###########################################################################
    #                             END OF YOUR CODE                            #
    ###########################################################################
    return dx
...
def spatial_groupnorm_forward(x, gamma, beta, G, gn_param):
    ...
    ###########################################################################
    # ref:https://blog.csdn.net/tech0ne/article/details/80619119
    N,C,H,W = x.shape
