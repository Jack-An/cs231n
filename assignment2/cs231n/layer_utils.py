...
def affine_bn_relu_forward(x, w, b, gamma, beta, bn_param):
    """
    Forward pass for the affine-bn-convenience layer
    """
...
def affine_bn_relu_backward(dout,cache):
    ...
    """
    fc_cache,bn_cahce,relu_cache = cache
    dx = relu_backward(dout,relu_cache)
    dx,dgamma,dbeta = batchnorm_backward_alt(dx,bn_cahce)
    dx,dw,db = affine_backward(dx,fc_cache)
    return dx,dw,db,dgamma,dbeta

def affine_ln_relu_forward(x, w, b, gamma, beta, ln_param):
    """
    Forward pass for the affine-ln-convenience layer
    """
    x,fc_cache = affine_forward(x,w,b)
    ln,ln_cache = layernorm_forward(x,gamma,beta,ln_param)
